...
def convert_csv_to_jsonl(csv_file_path: str, prompt: str) -> str:
    ...
    """
    # todo: copy and optimize your code from csv2json.py.
    pass
...
def save_job_result(output_file_id) -> str:
    ...
    result_file_path = "output_data/result.jsonl"  # or "output_data/result.csv", if you prefer
    for raw_response in raw_responses:
        json_response = json.loads(raw_response)
        formatted_json = json.dumps(json_response, indent=2)
        print(formatted_json)
        # todo: save the result to a file in /output_data
    return result_file_path


def join_results_with_original_data(csv_file_path: str, result_file_path: str):
    # todo: there should be a one-to-one mapping between the input csv file and the output result file so that we can match the results with the original data
    pass


if __name__ == "__main__":
    # todo: use sample data first, after testing, we can change to full data
    csv_file_path = "input_data/golf_course_reviews_sample.csv"
    prompt_templates = [
        """
            You are a sentiment analysis model, please classify the sentiment of this text as either 'positive' or 'negative' 
            or 'neutral'. Just answer one word:
        """,
    ]
